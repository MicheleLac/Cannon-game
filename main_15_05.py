...
class Tank(Widget):
    ...
    def __init__(self, **kwargs):
        ...
        self.laser_last_shot_time = 0.0  # Track the time of the last shot of laser (not changed in laser)
        self.bombshell_cooldown = 0.5 # added bombshell cooldown
        self.bombshell_last_shot_time = 0.0  # Track the time of the last shot of bombshell
...
def random_except(start, stop, exclude1, exclude2, exclude3):
    ...
    if (value > (exclude1 + 180) or value < (exclude1 - 180)) and ( value > (exclude2 + 180) or value < (exclude2 - 180)) and ( value > (exclude3 + 180) or value < (exclude3 - 280)):
            return value
    else:
        ...
        for bullet in self.bullets:
            ...
            if self.rock.collide_with_bullet(bullet):
                if isinstance(bullet, Bombshell):
                    ...
                    """ if bullet.explode(2) == True:
                     #we need somehow to say that after object is removed there is no collision between rock and anything else, because rock remembers it
            ...
            if self.mirror.collide_with_bullet(bullet): 
                if isinstance(bullet, Laser):
                    ...
                    def code_to_execute(dt):
                        ...
                        self.mirror.move()
                    Clock.schedule_once(bullet.explode, 2) 
                    Clock.schedule_once(code_to_execute, 3)
                    Clock.schedule_once(partial(self.remove_with_delay, bullet), 3 ) #fix 
                    """
                    if self.mirror.collide_with_bullet(bullet) and bullet.explode(2) == True:
                        #Clock.schedule_once(code_to_execute,2)
                        #Clock.schedule_once(self.mirror.move,2)
                    ...
                    #bullets_to_remove.add(bullet) #we need somehow to say that after object is removed there is no collision between rock and anything else, because rock remembers it
                    #self.mirror.move()
                    #self.counter.score()
